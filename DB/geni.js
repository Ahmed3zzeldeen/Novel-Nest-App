...
data.forEach(element => {
    var imageUrl = "https://covers.openlibrary.org/b/isbn/" + element.isbn + "-L.jpg";
    element.thumbnailUrl = imageUrl;
    let temp = {ISBN:element.isbn , author:element.authors[0] , bookTitle: element.title , price : (Math.floor(Math.random() * (250 - 100 + 1)) + 100) , numOfPages: (element.pageCount)? element.pageCount : (Math.floor(Math.random() * (550 - 100 + 1)) + 100)  , rete :  (Math.floor(Math.random() * (5 - 2 + 1)) + 2)  , category: (element.categories.length ===0 )?  "programming": element.categories[0] }  
    afterData.push(temp);
	
});
const jsonString = JSON.stringify(afterData);
fs.writeFile('books.json', jsonString, 'utf8', (err) => {
  if (err) {
